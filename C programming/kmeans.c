...
void* clustering(void* arg){
    ...
    int old_cluster = data[*k].cluster;
    //assigning the the cluster by clling teh function
    int new_cluster = get_closest_centroid(*k);
    data[*k].cluster = new_cluster; // Assign a cluster to the point i

    // int *change = malloc(sizeof(int));

    if (old_cluster != new_cluster) *k = 1;
    else *k = 0;
...
int kmeans(int k)
{
    ...
    // memeory allocation using malloc.
    ...
    do {
        ...
        for(int i = 0; i < N; i++){
             *d = i;
            // argumenta passing in pthreads. 
            pthread_create(&threads[i], NULL, clustering, (void*)(d));
            // joining the pthreads to achieve the synchronization.
...
{ 
    ...
    //printf("%s", ctime(&clk));
    sprintf(kmeansfile, "kmeans_output%s.txt",ctime(&clk) );
    FILE* fp = fopen(kmeansfile, "w");
    if (fp == NULL) {
        perror("Cannot open the file");
        exit(EXIT_FAILURE);
    }
